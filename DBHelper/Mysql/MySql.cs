...
namespace GapzLib.DBHelper
{
    public class MySql
    {
        ...
        public static string defaultConnectionString
        {
            get
            {
                if (string.IsNullOrEmpty(Host)) throw new NullReferenceException(Host);
                if (string.IsNullOrEmpty(DatabaseName)) throw new NullReferenceException(DatabaseName);
                if (string.IsNullOrEmpty(Users)) throw new NullReferenceException(Users);
                if (string.IsNullOrEmpty(Password)) throw new NullReferenceException(Password);

                return string.Format("server={0}; user={2}; database={1}; password={3};", Host, DatabaseName, Users, Password);
            }
        }
